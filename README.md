...
👁️ The Perception Module (Eyes): A YOLOv9 Convolutional Neural Network trained to detect and count local vehicles in real-time.

🧠 The Decision Module (Brain): A PyTorch-based Deep Q-Network (DQN) that takes vehicle counts as a state vector and calculates the optimal traffic light switches to minimize wait times and prioritize emergency vehicles.
